...
Empirical validation on a live Sovereign Cloud cluster (`kcc-cluster` in project `eu0:kcc-eval-de`, region `u-germany-northeast1`, universe `apis-berlin-build0.goog`) surfaced four structural defects:
...
3. **Workload Identity & Service Account Identity Mapping Failures:**  
   In sovereign partitions, project IDs are qualified with partition prefixes (e.g., `eu0:kcc-eval-de`). Google IAM formats the resulting Service Account emails with dot notation rather than colon notation (`sa@kcc-eval-de.eu0.iam.gserviceaccount.com`). KCC's internal IAM parser reconstructs emails assuming standard commercial syntax (`sa@eu0:kcc-eval-de.iam.gserviceaccount.com`), which contains an illegal RFC 5322 domain character (`:`) and triggers immediate API rejection.

4. **Static Monolithic CRD Deployment vs. Bounded Regional Capabilities:**  
   KCC operator unconditionally registers 295 CRDs during bootstrap. Telemetry from `serviceusage.googleapis.com` in `u-germany-northeast1` confirms that only 33 APIs are available in this sovereign boundary. Deploying 262 unsupported CRDs causes continuous reconciliation retry loops, metric export failures, and etcd bloat.
...
1. **No Regional Hardcoding:** Under no circumstances will Sovereign Cloud Germany domain names (`apis-berlin-build0.goog`), project prefixes (`eu0:`), or regional identities be hardcoded into KCC. All logic must consume `GOOGLE_CLOUD_UNIVERSE_DOMAIN` or the `universeDomain` spec field dynamically.
