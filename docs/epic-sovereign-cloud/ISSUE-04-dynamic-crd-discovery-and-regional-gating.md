...
Empirical scanning via `serviceusage.googleapis.com` in Google Sovereign Cloud Germany (`u-germany-northeast1`) revealed that only **33 services** exist in the universe, including:
...
4. **Declarative Mode via ConfigConnector Spec:**  
   ...
   spec:
     mode: cluster
     googleServiceAccount: "kcc-system@kcc-eval-de.eu0.iam.gserviceaccount.com"
     crdManagement:
...
- [ ] In Sovereign Germany (`apis-berlin-build0.goog`), only the ~33 supported service CRDs are registered when `crdManagement.discovery: Auto` is enabled.
